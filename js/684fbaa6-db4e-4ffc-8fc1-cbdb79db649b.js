const config = {
    ...
    extra_time_added_to_each_payment: 300000
}
...
const shapes = {
    ...
    tallTriangle: {
        d: "M3,3 L492,3 L492,500 L246,640 L3,500 Z",
        charX: '50%',
        charY: '50%',
        fontSize: '400',
        free: false,
...
const colorPalettes = {
    ...
    1: {
        colors: ['ff0000', 'ff8700', 'ffd60a', '97e519', '09cd44', '2d8be3', '391fff', 'be0aff'],
        free: false,
        name: 'Positive energy'
    },
    2: {
        colors: ['8338ec', 'c11cad', 'ff006e', 'fb5607', 'ffbe0b', '10bd58', '3a86ff', '2a40e8'],
        free: false,
        name: 'Happy colors'
    },
    3: {
        colors: ['c087e6', 'ff75aa', 'ffd15e', 'f9ff8a', 'b6e66e', '65cf93', '7ad9ff', '6698e8'],
        free: false,
        name: 'Pastelove'
    },
    4: {
        colors: ['c6baff', 'ffc4ff', 'ffa3cd', 'ffd6a5', 'fdffb6', 'caffbf', '9cf7ff', 'a4d3f5'],
        free: false,
        name: 'Light neon'
    },
    5: {
        colors: ['ba118a', 'f022a4', 'ff73c9', 'ffb8e8', 'ff99e7', 'ed5fd5', 'd100b5', 'a61797'],
        free: false,
        name: 'All pink'
    },
    6: {
        colors: ['c9e8f3', '90bcda', '4991d0', '1271bf', '2153e8', '0a36e6', '0423bf', '051399'],
        free: false,
        name: 'Winter time',
    },
    7: {
        colors: ['fcff5e', 'fcef04', 'c9d408', '95b80d', '629d11', '2f8216', '126615', '175210'],
        free: false,
        name: 'Healthy spring'
    },
    8: {
        colors: ['be0aff', 'ff0000', 'ff8700', 'ffd60a', '0ac419', '127a14', '0099ff', '0020ed'],
        free: false,
        name: 'Intense summer'
    },
    9: {
        colors: ['85201b', 'c2251d', 'd8572a', 'db7c26', 'f7b538', '18a413', '247d24', '0a5905'],
        free: false,
        name: 'Colorful fall'
    },
    10: {
        colors: ['5f0f40', '9a031e', 'dd3000', 'de6114', 'fb8b24', '62a341', '1c7b47', '1a4f5c'],
        free: false,
        name: 'Late fall'
    },
    12: {
        colors: ['f7b538', 'db7c26', 'd8572a', 'c2251d', '9c1a13', '752b27', '3e833e', '18a413'],
        free: false,
        name: 'Indian summer',
        order: 'FIXED'
    },
    13: {
        colors: ['ffba08', 'f48c06', 'e85d04', 'dc2f02', 'c40000', '9d0208', '6a040f', '370617'],
        free: false,
        name: 'Tailgate picnic',
        order: 'FIXED'
    },
    14: {
        colors: ['ffd363', 'f0b13c', 'e8910e', 'cc7802', 'eb7323', 'c44800', '963102', '732005'],
        free: false,
        name: 'Little pumpkin',
    },
    15: {
        colors: ['e34649', 'f3722c', 'f79f34', 'f9c74f', '90be6d', '43aa8b', '1f7e7b', '386389'],
        free: false,
        name: 'Dim lights',
        order: 'FIXED'
    },
    ...
    17: {
        colors: ['e0ffde', 'b4edaf', '82e37b', '5bcf57', '2aad2a', '339c5b', '007d4f', '056336'],
        free: false,
        name: 'Fragrant fir',
    },
    18: {
        colors: ['ffdbdb', 'ffb0b0', 'ff8585', 'ff5252', 'ff0303', 'ed0000', 'd10202', 'b30000'],
        free: false,
        name: 'Santa\'s hat',
    },
    19: {
        colors: ['ff1919', 'ff6666', 'ffa6a6', 'ffd3c4', 'edb39d', 'd9785f', 'c63729', '990000'],
        free: false,
...
const createPage = (template, width, height, letter, color, shapeName, patternName) => {
    ...
    // prepare pattern
    const patternTemplate = patterns[patternName].def
...
const sendEvent_generate = (textToGenerate) => {
    sendEvent_clicked('generate_summary');
    sendEvent_clicked('generate_' + textToGenerate);
}

const sendEvent_clicked = (postfix) => {
    sendEvent('clicked-' + postfix)
}

const sendEvent = (eventValue) => {
...
const prepareViewData = () => {
    ...
    const patternToShow = Object.getOwnPropertyNames(patterns).map(
        patternName => ({
            name: patternName,
            free: patterns[patternName].free,
...
var app = new Vue({
    ...
    data: {
        ...
        payment: {
            // OPTIONS, PROCESS, SUCCESS
            show: 'OPTIONS',
            periodTitle: '',
            payPalPaymentButtons: undefined,
            bootstrapModal: undefined,
            codeRedeemInProgress: false,
            alert: {
                warn: '',
                error: ''
            },
            enableTwitter: true,
            enableTwitterPayOnlyOnce: false // możliwość wyłączenia na potrzeby promowania
        },
        seinfeld: false,
    ...
    methods: {
        validateBannerText() {
            ...
            if (this.bannerValue.length === 0) {
                inputEl.classList.add('is-invalid');
                inputEl.scrollIntoView({ block: "center" });
                this.input_generateText_error = 'Banner text cannot be empty'
                return false
            } else {
                inputEl.classList.remove('is-invalid')
                inputEl.classList.add('is-valid')
                this.input_generateText_error = ''
                return true
            }
        },

        showPurchaseModal: function () {
            const htmlModal = document.getElementById('purchaseModal')
            const purchaseModal = new bootstrap.Modal(htmlModal, {});
            this.payment.bootstrapModal = purchaseModal
            htmlModal.addEventListener('hide.bs.modal', function () {
                app.payment.show = 'OPTIONS'
                app.payment.alert.warn = ''
                app.payment.alert.error = ''
                if (app.payment.payPalPaymentButtons) {
                    app.payment.payPalPaymentButtons.close()
                }
            })
            purchaseModal.show();
            sendEvent_clicked('payment_modalOpened')
        },
        ...
        generate: function () {
            if (!this.validateBannerText(this.bannerValue)) {
                return
            }

            this.generatingInProgress = true;

            sendEvent_generate(this.bannerValue);
        ...
        payWithTweet: function () {
            // generate and save code
            const nowEpoch = Date.now() + 1000 * 60 * 60;
            const b = nowEpoch - (nowEpoch % 60000);
            const c = btoa(b.toString(36)).replace('=', '@')
            this.applyCode(c)

            // mark twitter as used
            localStorage['2d2d9310-c4bc-4de2-8d34-2428d2faa0ee'] = '764e52a4-47a0-11ec-81d3-0242ac130003'

            window.location.href = "http://twitter.com/intent/tweet?text=Free banners generator, create your own!&url=www.bigdaybanner.com&hashtags=bigdaybanner," + nowEpoch

            sendEvent_clicked('payment_payWithTweet')
        },
            ...
            const paypalProps = {
                ...
                onApprove: function (data, actions) {
                    return actions.order.capture().then(function (orderData) {
                        const validUntilMs = Date.now() + periodMs + config.extra_time_added_to_each_payment
                        const validUntilMins = validUntilMs - (validUntilMs % 60000)
                        const validUntil36 = btoa(validUntilMins.toString(36)).replace('=', '@')
                        const applyCodeResult = app.applyCode(validUntil36)
                        if (applyCodeResult === 0) {
                            app.showCodeApplySuccessful();
                        }
                    });
                },
                onCancel: function (data) {
                    app.showPaymentCancel();
                },
                onError: function (err) {
                    console.error(err)
                    app.showPaymentError();
                }
            }

            this.payment.show = 'PROCESS';
            this.payment.payPalPaymentButtons = paypal.Buttons(paypalProps);
            this.payment.payPalPaymentButtons.render('#paypalContainer');

            sendEvent_clicked('payment_paymentOption_' + paymentOption)
        },

        closePayment: function () {
            this.payment.show = 'OPTIONS';
        ...
        applyCodeInput: function () {
            ...
            this.payment.codeRedeemInProgress = true;
            ...
            setTimeout(() => {
                ...
                this.payment.codeRedeemInProgress = false;
            }, 2000);

            sendEvent_clicked('payment_applyCodeInput')
        },
        ...
        applyCode: function (inputCode) {
            try {
                ...
                // in future but not later then 31 days in the future
        ...
        showCodeApplySuccessful: function () {
            this.payment.alert.warn = ''
            this.payment.alert.error = ''
            this.payment.show = 'SUCCESS';
        },

        showPaymentError: function () {
            this.payment.alert.error = 'Payment provider returned an error. <br/>In case of any concerns, please contact us at hello@bigdaybanner.com'
        },
    ...
    mounted: function () {
        // twitter payment (disable if used already)
        if (localStorage['2d2d9310-c4bc-4de2-8d34-2428d2faa0ee']) {
            if (this.payment.enableTwitterPayOnlyOnce) {
                this.payment.enableTwitter = false
            }
        }
        ...
        if (inputCode) {
            const applyResult = this.applyCode(inputCode)
            if (applyResult === -1) {
                localStorage.removeItem('403e433e-5e0e-46bc-995e-fcf2691ffe35');
            }
            if (applyResult === -2) {
                this.showToast("Your code has expired")
                localStorage.removeItem('403e433e-5e0e-46bc-995e-fcf2691ffe35');
            }
            if (applyResult === -99) {
                this.showToast("Unexpected error, please contact us at hello@bigdaybanner.com.")
            }
        }
